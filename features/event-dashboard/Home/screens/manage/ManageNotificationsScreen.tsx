...
type EventNotificationChannel = 'EMAIL' | 'SMS' | 'PUSH' | 'IN_APP';
...
const INITIAL_REMINDERS: EventReminder[] = [
  {
    reminderId: 'rem-1',
    title: 'Doors Open',
    reminderTime: new Date(Date.now() + 1000 * 60 * 60 * 2).toISOString(),
    channel: 'PUSH',
    customMessage: 'Doors open in 2 hours. See you soon!',
  },
  {
    reminderId: 'rem-2',
    title: 'Event Starts',
    reminderTime: new Date(Date.now() + 1000 * 60 * 60 * 4).toISOString(),
    channel: 'EMAIL',
    customMessage: 'Your event starts soon. Check your schedule.',
  },
];
...
const ManageNotificationsScreen = () => {
  ...
  // Reminder state
  const [reminders, setReminders] = useState<EventReminder[]>(INITIAL_REMINDERS);
  ...
  const load = useCallback(() => {
    setRefreshing(true);
    setSendForm(prev => ({ ...prev, channel: 'EMAIL' }));
  ...
  const populateReminder = useCallback((reminder: EventReminder) => {
  ...
  const handleSaveReminder = useCallback(async () => {
    ...
    const saved: EventReminder = {
      reminderId: editingReminder?.reminderId ?? `rem-${Date.now()}`,
      title: reminderForm.title,
      reminderTime: reminderForm.dateTime.toISOString(),
      channel: reminderForm.channel as EventNotificationChannel,
  ...
  }, [editingReminder, reminderForm, resetReminderForm]);
  ...
  const handleDeleteReminder = useCallback(
    ...
    [editingReminder?.reminderId, resetReminderForm],
  );
