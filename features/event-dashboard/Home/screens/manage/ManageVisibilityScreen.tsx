...
const ManageVisibilityScreen = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCurrentStatus(visibilityData?.eventStatus ?? eventAny?.eventStatus ?? null);
      setIsPublic(
  ...
  const handleUpdateStatus = useCallback(async () => {
    ...
    setBusy(true);
    try {
      await eventService.updateEvent(eventId, { event: { eventStatus: selectedStatus } });
  ...
  const handleQuickStatusAction = useCallback(
    async (action: 'publish' | 'complete' | 'cancel') => {
          ...
          [
            { text: 'No', style: 'cancel' },
            {
              ...
              onPress: async () => {
                setBusy(true);
                try {
                  await eventService.updateEvent(eventId, { event: { eventStatus: EventStatus.CANCELLED } });
                  await load();
                  Alert.alert('Success', 'Event status updated successfully.');
                } catch (error) {
                  Alert.alert(
      ...
      try {
        if (action === 'publish') {
          await eventService.updateEvent(eventId, { event: { eventStatus: EventStatus.PUBLISHED } });
        } else {
          await eventService.updateEvent(eventId, { event: { eventStatus: EventStatus.COMPLETED } });
        }
        await load();
        Alert.alert('Success', 'Event status updated successfully.');
      } catch (error) {
