...
import { EventCollaboratorRequest, EventCollaboratorResponse } from '../../../../../common/types';
...
const ManageCollaboratorsScreen = () => {
  ...
  const load = useCallback(async () => {
    try {
      const data = await eventService.getCollaborators(params.eventId);
  ...
  const handleAdd = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const payload: EventCollaboratorRequest = {
        email,
        role: role as EventCollaboratorRequest['role'],
      ...
      const result = await eventService.addCollaborator(params.eventId, payload);
  ...
  }, [email, invite, message, notes, params.eventId, permissions, role]);

  const handleRemove = useCallback(
    async (id: string) => {
      setBusy(true);
      try {
        await eventService.removeCollaborator(params.eventId, id);
