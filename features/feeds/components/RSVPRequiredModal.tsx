...
import { UserCheck, Calendar, MapPin, Users, ArrowRight } from 'lucide-react-native';
...
export type RSVPRequiredModalProps = {
  visible: boolean;
  eventId: string;
  eventName: string;
...
export const RSVPRequiredModal = ({
  visible,
  eventName,
  description,
  coverImageUrl,
        ...
        <View style={{ width, height: width * 0.55 }}>
          ...
          >
            <View className="flex-1 justify-between p-lg bg-light-overlay-soft dark:bg-dark-overlay-soft">
              ...
              >
                <UserCheck size={12} color={overlayIconColor} strokeWidth={2} />
                <Text className="text-xs font-medium text-txt-inverse ml-xs">{t('RSVPRequired')}</Text>
              </View>
        ...
        >
          {successMessage ? (
            <View className="rounded-md p-sm mb-sm bg-semantic-success-light dark:bg-semantic-success/20">
              <Text className="text-xs text-semantic-success">{successMessage}</Text>
            </View>
          ) : null}

          {errorMessage ? (
            <View className="rounded-md p-sm mb-sm bg-semantic-error-light dark:bg-semantic-error/[0.15]">
              <Text className="text-xs text-semantic-error">{errorMessage}</Text>
            </View>
          ) : null}
          ...
          >
            <Text className="text-base font-semibold text-txt-inverse dark:text-txt-dark-inverse">
              {loading ? t('Processing') : t('RSVPNow')}
            </Text>
            <View className="ml-1">
