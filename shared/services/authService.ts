...
export const authService = {
  ...
  async loginNew(request: LoginRequest) {
    ...
    if (res.data) {
      ...
      const { setRefreshToken, setDeviceId, setUser } = await import('../storage/authStorage');
  ...
  async refreshToken(request?: RefreshTokenRequest) {
    ...
    if (!request) {
      const { getRefreshToken, getDeviceId } = await import('../storage/authStorage');
    ...
    if (res.data) {
      ...
      if (res.data.refreshToken) {
        const { setRefreshToken } = await import('../storage/authStorage');
        await setRefreshToken(res.data.refreshToken);
      }

      // Update deviceId if provided
      if (res.data.deviceId) {
        const { setDeviceId } = await import('../storage/authStorage');
        await setDeviceId(res.data.deviceId);
      }
